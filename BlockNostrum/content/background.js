...
function find_in_whitelist(full_host)
{
    for (var key_host in whiteMap)
    {
        if (full_host.indexOf(key_host) >= 0)
            return whiteMap[hostObj];
    }
}
...
function updatePageAction(tabId,tabURL){
    ...
    if (!enabled && enablePageAction)
        chrome.pageAction.setIcon({tabId:tabId,path:"/icons/disabled.png"})
    var host=getHost(tabURL)
    var src = find_in_whitelist(host);
    var extSrc = externalWhiteListMap[host]
    if (extSrc){
        if (enabled && enablePageAction)
            chrome.pageAction.setIcon({tabId:tabId,path:"/icons/allowed.png"})		
        ...
        if (enabled && enablePageAction)
            chrome.pageAction.setIcon({tabId:tabId,path:"/icons/blocked.png"})
        return BLOCKED
    }
}

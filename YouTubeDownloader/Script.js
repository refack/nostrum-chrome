...
if ((document.getElementById('video-player') || document.getElementById('movie_player')) && !document.getElementById('embed-holder') && !document.getElementById('watch-download')) {
    if (document.getElementsByTagName('html').item(0).lang && document.getElementsByTagName('body').item(0).className && window.location.href.match('youtube.com')) {
        ...
        if (langh === 'da') {
            textlang = ['Hent denne film!', 'Hent film'];
        }
        if (langh === 'de') {
            textlang = ['Dieses YouTube video herunterladen', 'Download'];
        }
        if (langh === 'en') {
            textlang = ['Download this video!', 'Download'];
        }
        if (langh === 'es') {
            textlang = ['Descargar este vídeo!', 'Descargar'];
        }
        //if(langh === 'fi' && langb.match('fil_PH')){textlang = ['','']}
        if (langh === 'fr') {
            textlang = ['Télécharger cette vidéo!', 'Télécharger'];
        }
        if (langh === 'hr') {
            textlang = ['Preuzmite ovaj videa', 'Preuzmite'];
        }
        if (langh === 'it') {
            textlang = ['Scarica questo video!', 'Scarica'];
        }
        if (langh === 'lt') {
            textlang = ['Parsisiųsk šį vaizdo įrašą!', 'Parsisiųsti'];
        }
        if (langh === 'hu') {
            textlang = ['Videó letöltése', 'Letöltés'];
        }
        if (langh === 'nl') {
            textlang = ['Download deze video!', 'Download'];
        }
        if (langh === 'nb') {
            textlang = ['Last ned denne videoen!', 'Last ned'];
        }
        if (langh === 'pl') {
            textlang = ['Pobierz ten plik video!', 'Pobierz'];
        }
        if (langh === 'pt' && langb.match('pt_PT')) {
            textlang = ['Baixe este video!', 'Baixar'];
        }
        if (langh === 'pt' && langb.match('pt_BR')) {
            textlang = ['Baixar este vídeo!', 'Download'];
        }
        if (langh === 'ru') {
            textlang = ['Закачать видео', 'Закачать'];
        }
        if (langh === 'ro') {
            textlang = ['Descarca acest Video', 'Descarca'];
        }
        if (langh === 'sk') {
            textlang = ['Stiahnuť toto video!', 'Stiahnuť'];
        }
        //if(langh === 'sl'){textlang = ['','']}
        if (langh === 'fi' && langb.match('fi_FI')) {
            textlang = ['Tallenna tämä video', 'Tallenna'];
        }
        if (langh === 'sv') {
            textlang = ['Hämta det här videoklippet!', 'Hämta'];
        }
        if (langh === 'vi') {
            textlang = ['Tải xuống video này', 'Tải xuống'];
        }
        if (langh === 'cs') {
            textlang = ['Stáhněte si toto video!', 'Stáhnout'];
        }
        if (langh === 'el') {
            textlang = ['Κατεβάστε αυτό το βίντεο!', 'Download'];
        }
        if (langh === 'sr') {
            textlang = ['Skini ovaj video', 'Skini'];
        }
        if (langh === 'sq') {
            textlang = ['Shkarko këtë video!', 'Shkarko'];
        }
        if (langh === 'uk') {
            textlang = ['Завантажити це відео!', 'Завантаження'];
        }
        if (langh === 'bg') {
            textlang = ['Свали този клип', 'Свали'];
        }
        if (langh === 'iw') {
            textlang = ['הורד את הסרטון הזה!', 'הורדה'];
        }
        if (langh === 'ar') {
            textlang = ['تحميل هذا الفيديو', 'تحميل'];
        }
        if (langh === 'zh') {
            textlang = ['下載這部影片', '下載'];
        }
        if (langb.match('ja_JP')) {
            textlang = ['この動画をダウンロード！', 'ダウンロード'];
        }
        if (langh === 'ko') {
            textlang = ['이 비디오 다운로드!', '다운로드'];
        }
    }
